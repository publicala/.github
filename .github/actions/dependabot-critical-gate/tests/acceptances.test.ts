...
import { activeCandidateAcceptance, parseAcceptances } from '../src/acceptances.js';
...
test('rejects expired and overlong risk acceptances', () => {
  assert.throws(() => parseAcceptances(file('risk', { expires: '2026-09-03T11:59:59Z' }), now), /has expired/);
  assert.throws(
...
test('requires exact alert identity and a Linear URL', () => {
  ...
  assert.equal(activeCandidateAcceptance([], acceptance, { ...alert, ghsa: 'GHSA-9999-9999-9999' }, new Set([22])), false);
});
...
test('allows a new remediation only for its pull request and for 48 hours', () => {
  ...
  assert.equal(activeCandidateAcceptance([], remediation, alert, new Set([22])), true);
  assert.equal(activeCandidateAcceptance([], remediation, alert, new Set([23])), false);
  assert.equal(activeCandidateAcceptance(remediation, remediation, alert, new Set([22])), false);
  assert.throws(
